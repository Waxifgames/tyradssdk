...
namespace Plugins.TyrAdsSDK.Scripts.UI.Elements
{
    ...
    public class TA_ScrollWithInput : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        void SmoothVelocity()
        {
            curVelocity = Mathf.Lerp(lastVelocity, curVelocity, smoothingFactor * Time.deltaTime);
        ...
        {
            curVelocity *= Mathf.Pow(0.5f, Time.deltaTime / decelerateTimeX);

            if (Mathf.Abs(curVelocity) < 0.1f)
            {
                if (isDragging)
                {
                    isDragging = false;
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if ( detectDirTimer <= 0)
            {
                ...
                if (SwipeLeft || SwipeRight)
                {
                    ...
                    xVelocity = _curX - _lastX; // Get horizontal velocity
                }
                else
                {
