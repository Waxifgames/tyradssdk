...
using Plugins.IngameDebugConsole.Android;
#endif
using UnityEngine;
...
#pragma warning disable 0649
        ...
        bool resizeFromRight = true;

        [SerializeField]
        [HideInInspector]
        [Tooltip("Minimum width of the console window")]
        float minimumWidth = 240f;
 ...
 new InputAction( "Toggle Binding", type: InputActionType.Button, binding: "<Keyboard>/backquote", expectedControlType: "Button" );
        ...
        float topSearchbarMinWidth = 360f;
        ...
        bool receiveLogsWhileInactive;

        [SerializeField]
        [HideInInspector]
        bool receiveInfoLogs = true,
        ...
        bool clearCommandAfterExecution = true;
        ...
        bool receiveLogcatLogsInAndroid;
...
#pragma warning restore 0414
        ...
        bool autoFocusOnCommandInputField = true;
#endif

        [Header("Visuals")]
        [SerializeField]
        DebugLogItem logItemPrefab;

        [SerializeField] Text commandSuggestionPrefab;

        // Visuals for different log types
        [SerializeField] Sprite infoLog;
        [SerializeField] Sprite warningLog;
        [SerializeField] Sprite errorLog;

        // Visuals for resize button
        [SerializeField] Sprite resizeIconAllDirections;
        [SerializeField] Sprite resizeIconVerticalOnly;

        Dictionary<LogType, Sprite> logSpriteRepresentations;

        [SerializeField] Color collapseButtonNormalColor;
        [SerializeField] Color collapseButtonSelectedColor;

        [SerializeField] Color filterButtonsNormalColor;
        [SerializeField] Color filterButtonsSelectedColor;

        [SerializeField] string commandSuggestionHighlightStart = "<color=orange>";
        [SerializeField] string commandSuggestionHighlightEnd = "</color>";

        [Header("Internal References")]
        [SerializeField]
        RectTransform logWindowTR;

        internal RectTransform canvasTR;

        [SerializeField] RectTransform logItemsContainer;

        [SerializeField] RectTransform commandSuggestionsContainer;
        ...
        [SerializeField] Image collapseButton;

        [SerializeField] Image filterInfoButton;
        [SerializeField] Image filterWarningButton;
        [SerializeField] Image filterErrorButton;

        [SerializeField] Text infoEntryCountText;
        [SerializeField] Text warningEntryCountText;
        [SerializeField] Text errorEntryCountText;

        [SerializeField] RectTransform searchbar;
        [SerializeField] RectTransform searchbarSlotTop;
        [SerializeField] RectTransform searchbarSlotBottom;

        [SerializeField] Image resizeButton;

        [SerializeField] GameObject snapToBottomButton;

        // Canvas group to modify visibility of the log window
        [SerializeField] CanvasGroup logWindowCanvasGroup;

        [SerializeField] DebugLogPopup popupManager;

        [SerializeField] ScrollRect logItemsScrollRect;
        RectTransform logItemsScrollRectTR;
        Vector2 logItemsScrollRectOriginalSize;

        // Recycled list view to handle the log items efficiently
        [SerializeField] DebugLogRecycledListView recycledListView;
#pragma warning restore 0649

        bool isLogWindowVisible = true;
        ...
        bool screenDimensionsChanged = true;
        float logWindowPreviousWidth;

        // Number of entries filtered by their types
        int infoEntryCount, warningEntryCount, errorEntryCount;
        bool entryCountTextsDirty;
        ...
        bool isCollapseOn;
        DebugLogFilter logFilter = DebugLogFilter.All;
        ...
        List<DebugLogEntryTimestamp> collapsedLogEntriesTimestamps;

        // Dictionary to quickly find if a log already exists in collapsedLogEntries
        Dictionary<DebugLogEntry, int> collapsedLogEntriesMap;

        // The order the collapsedLogEntries are received 
        // (duplicate entries have the same index (value))
        DebugLogIndexList<int> uncollapsedLogEntriesIndices;
        DebugLogIndexList<DebugLogEntryTimestamp> uncollapsedLogEntriesTimestamps;

        // Filtered list of debug entries to show
        DebugLogIndexList<int> indicesOfListEntriesToShow;
        DebugLogIndexList<DebugLogEntryTimestamp> timestampsOfListEntriesToShow;

        // The log entry that must be focused this frame
        int indexOfLogEntryToSelectAndFocus = -1;

        // Whether or not logs list view should be updated this frame
        bool shouldUpdateRecycledListView;

        // Logs that should be registered in Update-loop
        DynamicCircularBuffer<QueuedDebugLogEntry> queuedLogEntries;
        DynamicCircularBuffer<DebugLogEntryTimestamp> queuedLogEntriesTimestamps;
        object logEntriesLock;
        int pendingLogToAutoExpand;

        // Command suggestions that match the currently entered command
        List<Text> commandSuggestionInstances;
        int visibleCommandSuggestionInstances;
        List<ConsoleMethodInfo> matchingCommandSuggestions;
        List<int> commandCaretIndexIncrements;
        string commandInputFieldPrevCommand;
        string commandInputFieldPrevCommandName;
        int commandInputFieldPrevParamCount = -1;
        int commandInputFieldPrevCaretPos = -1;
        int commandInputFieldPrevCaretArgumentIndex = -1;

        // Value of the command input field when autocomplete was first requested
        string commandInputFieldAutoCompleteBase;
        bool commandInputFieldAutoCompletedNow;

        // Pools for memory efficiency
        List<DebugLogEntry> pooledLogEntries;
        List<DebugLogItem> pooledLogItems;

        // History of the previously entered commands
        CircularBuffer<string> commandHistory;
        ...
        System.TimeSpan localTimeUtcOffset;
        ...
        DebugLogEntryTimestamp dummyLogEntryTimestamp;
        ...
        char OnValidateCommand(string text, int charIndex, char addedChar)
        {
        ...
        void ProcessQueuedLogs(int numberOfLogsToProcess)
        {
        ...
        void ProcessLog(QueuedDebugLogEntry queuedLogEntry, DebugLogEntryTimestamp timestamp)
        {
        ...
        void FilterWarningButtonPressed()
        {
        ...
        void OnEditCommand(string command)
        {
        ...
        void OnEndEditCommand(string command)
        {
            if (commandSuggestionsContainer.gameObject.activeSelf)
                commandSuggestionsContainer.gameObject.SetActive(false);
        }
...
#if UNITY_EDITOR || UNITY_STANDALONE || UNITY_WEBGL
        IEnumerator ActivateCommandInputFieldCoroutine()
        {
